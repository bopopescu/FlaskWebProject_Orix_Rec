...
class Mysql3:
    ...
    def get_v(self):
        ...
        normal_list_sql = "select * from `Drive_recorder` WHERE `DATE` BETWEEN ('" + target_normal_time + "' - INTERVAL " + str(self.normal_list_between1) + " SECOND) AND ('" + target_normal_time + "' + INTERVAL " + str(self.normal_list_between2) + " SECOND) AND sub = " + str(self.notmal_target_info['sub'])
        normal_v_list =  self.mysql.sql_excute_fetch(normal_list_sql)
    ...
    def regist_row3(self, regist_name, regist_value):
        sql = "INSERT INTO `"+self.target_table+"` (`" +regist_name+ "`) VALUES ('"+regist_value+"') ON DUPLICATE KEY UPDATE `" +regist_name+ "` = VALUES (`" +regist_name+ "`)";
        #sql = "UPDATE " + target_table + " SET " + regist_name + " = " + regist_value + " WHERE id=" + str(id)
        #print("sql")
        #print(sql)
        self.cur.execute(sql)
    ...
    def change_target_id(self, new_id):
    ...
    def regist_normal_list(self):
        self.mysql.regist_init('normal_list')
        self.mysql.regist_row('DATE', str(self.notmal_target_info['DATE']))
        self.mysql.regist_row('sub', str(self.notmal_target_info['sub']))
        self.mysql.regist_row('latitude', str(self.notmal_target_info['latitude']))
        self.mysql.regist_row('longitude', str(self.notmal_target_info['longitude']))
        self.mysql.regist_row('type', str(self.normal_type))
        self.mysql.regist_row('delta_v', str(self.delta_v))
        self.mysql.regist_row('t_period', str(self.normal_v_len))
        if(self.g):
            self.mysql.regist_row('g', str(self.g))

        #print(type(self.g_box))
        if(self.g_box):
            self.mysql.regist_row('g_max', str(max(self.g_box)))
            self.mysql.regist_row('g_min', str(min(self.g_box)))



        sql = "SELECT * FROM `hazard_list` WHERE  `DATE2` = '" + str(self.notmal_target_info['DATE']) + "' AND sub = " + str(self.notmal_target_info['sub'])
        #print(sql)
        var = self.mysql.sql_excute_fetch(sql)
        #print(var)
        #print(self.notmal_target_info)

        if var:
            #print(var[0]['id'])
            self.mysql.regist_row('normal_list_id_FLAG', '1')
            self.mysql.regist_init('hazard_list')
            self.mysql.change_target_id(var[0]['id'])
            self.mysql.regist_row('normal_list_id', str(self.notmal_target_info['id']))
        else:
            self.mysql.regist_row('normal_list_id_FLAG', '0')




    def regist_normal_list_v(self):
        self.mysql.regist_init('normal_list_v')
        #print(self.normal_v_list)
        for i, row_info in enumerate(self.normal_v_list, 1):
            ...
            if(str(row_info['DATE']) == str(self.notmal_target_info['DATE']) ):
                if(i>1):
                    self.g = float(self.normal_v_list[i-1]['velocity_kmh'] - self.normal_v_list[i-2]['velocity_kmh'])/3.6/9.8
                    self.mysql.regist_row2('t', str(i), 'g', str(self.g))
                self.mysql.regist_row2('t', str(i), 'target_flag', str(1))
            else:
                self.mysql.regist_row2('t', str(i), 'target_flag', str(0))
            self.mysql.regist_row2('t', str(i), 'DATE', str(row_info['DATE']))
            self.mysql.regist_row2('t', str(i), 'v', str(row_info['velocity_kmh']))
            self.mysql.regist_row2('t', str(i), 'latitude', str(row_info['latitude']))
            self.mysql.regist_row2('t', str(i), 'longitude', str(row_info['longitude']))

            if(i>1):
                a_m_ss = float(self.normal_v_list[i-1]['velocity_kmh'] - self.normal_v_list[i-2]['velocity_kmh'])/3.6
                self.mysql.regist_row2('t', str(i), 'a', str(a_m_ss))
                self.mysql.regist_row2('t', str(i), 'g', str(a_m_ss/9.8))
    ...
    def get_notmal_target_info(self, id):
        ...
        #print(self.mysql)
        self.notmal_target_info =  self.mysql.sql_excute_fetch(normal_list_sql)[0]
        #print(self.notmal_target_info)

    def create_normal_list(self, mysql3):
        #print('mysql3')
        #print(mysql3)
        ...
        #print(self.target_id)
        #aaa
        mysql3.get_notmal_target_info(self.target_id)
        mysql3.get_v()
        mysql3.regist_normal_list_v()
        mysql3.regist_normal_list()
...
if __name__ == '__main__':
    ...
    mysql = Mysql3(host='172.31.19.191', db_name='orix_data', user='root',normal_list_between1=5, normal_list_between2=5, target_sub=1, target_id=10)
    ...
    for i in tqdm(range(1,33792)):
        ...
        """
       SELECT * FROM normal_list
       INNER JOIN hazard_list
        ON normal_list.DATE = hazard_list.DATE2 AND normal_list.sub = hazard_list.sub;
        """
